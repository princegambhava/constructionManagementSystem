...
const StaffAssignmentCard = ({
  ...
  const handleSiteManagerChange = (siteManagerId) => {
    setSelectedSiteManagers((prev) =>
      prev.includes(siteManagerId)
        ? prev.filter((id) => id !== siteManagerId)
        : [...prev, siteManagerId],
    );
  };

  const handleAssignEngineers = () => {
    console.log("🔍 StaffAssignmentCard - Assigning engineers to project:", project);
    console.log("🔍 StaffAssignmentCard - Project._id:", project._id);
    onAssignEngineers(project._id, selectedEngineers);
  };

  const handleAssignSiteManagers = () => {
    console.log("🔍 StaffAssignmentCard - Assigning site managers to project:", project);
    console.log("🔍 StaffAssignmentCard - Project._id:", project._id);
    onAssignSiteManagers(project._id, selectedSiteManagers);
  };
  ...
  return (
    <div className="glass-card p-6 hover:shadow-lg transition-shadow">
      ...
      {/* Site Manager Assignment Section */}
      <div className="border-t pt-4">
        ...
        <div className="space-y-2 mb-3">
          {siteManagers.length === 0 ? (
            <p className="text-sm text-gray-500">No site managers available</p>
          ) : (
            siteManagers.map((siteManager) => (
              ...
              >
                <input
                  type="checkbox"
                  checked={selectedSiteManagers.includes(siteManager._id)}
                  onChange={() => handleSiteManagerChange(siteManager._id)}
